"""Tests for the singularity-memory provider."""
...
def _create_initialized_provider(hermes_home: Path) -> SingularityMemoryProvider:
...
def test_search_tool_returns_stored_memory_when_query_matches(tmp_path: Path) -> None:
    ...
    response = json.loads(provider.handle_tool_call("singularity_memory_search", {"query": "password rotates"}))
...
def test_context_tool_honors_token_budget_when_results_are_formatted(tmp_path: Path) -> None:
    ...
    response = json.loads(provider.handle_tool_call("singularity_memory_context", {"query": "deployment database", "token_budget": 8}))
...
def test_sync_turn_persists_completed_turn_into_searchable_memory(tmp_path: Path) -> None:
    ...
    response = json.loads(provider.handle_tool_call("singularity_memory_search", {"query": "retry logging"}))
...
def test_session_end_stores_summary_as_searchable_memory(tmp_path: Path) -> None:
    ...
    response = json.loads(provider.handle_tool_call("singularity_memory_search", {"query": "wrong service"}))
