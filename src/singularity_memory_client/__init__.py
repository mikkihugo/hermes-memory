"""
Singularity Memory Client - pythonic wrapper for the Singularity Memory server.

This package provides a high-level ``SingularityMemoryClient`` class with simplified methods
...
clients exposed as properties on the ``SingularityMemoryClient`` instance (e.g.
...
Quick start::
    ...
    client = SingularityMemoryClient(base_url="http://localhost:8888")
...
from singularity_memory_client_api.models.recall_response import RecallResponse as _RecallResponse
from singularity_memory_client_api.models.recall_result import RecallResult as _RecallResult
...
def _recall_response_to_prompt_string(self) -> str:
    ...
    This mirrors the format used internally by the reflect operation.
    """
    import json
