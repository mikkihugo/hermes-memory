"""
Clean, pythonic wrapper for the Singularity Memory server client.
...
import singularity_memory_client_api

try:
    _CLIENT_VERSION = metadata.version("singularity-memory")
except metadata.PackageNotFoundError:
    _CLIENT_VERSION = "0.0.0"

DEFAULT_USER_AGENT = f"singularity-memory-client-python/{_CLIENT_VERSION}"
from singularity_memory_client_api.api import (
    banks_api,
    directives_api,
    documents_api,
    entities_api,
...
from singularity_memory_client_api.models import (
    memory_item,
    recall_request,
    reflect_request,
    retain_request,
...
class SingularityMemoryClient:
    """
    High-level, easy-to-use Singularity Memory client.
    ...
    **Prefer the async variants** (``aretain``, ``arecall``, ``areflect``, etc.)
    ...
    Example — async (preferred)::
        ...
        client = SingularityMemoryClient(base_url="http://localhost:8888", api_key="your-api-key")
    ...
    def __init__(
        ...
        """
        Initialize the Singularity Memory client.

        Args:
            base_url: The base URL of the Singularity Memory server server
            ...
            user_agent: Override the default ``User-Agent`` header. Integrations
                ...
                ``"singularity-memory-crewai/1.2.0"``). Defaults to
                ``singularity-memory-client-python/<version>``.
        """
        config = singularity_memory_client_api.Configuration(host=base_url, access_token=api_key)
        self._api_client = singularity_memory_client_api.ApiClient(config)
    ...
    async def arecall(
        ...
        if tag_groups is not None:
            from singularity_memory_client_api.models.recall_request_tag_groups_inner import RecallRequestTagGroupsInner
    ...
    def create_mental_model(
        ...
        """
        from singularity_memory_client_api.models import create_mental_model_request, mental_model_trigger_input
    ...
    def update_mental_model(
        ...
        Returns:
            MentalModelResponse
        """
        from singularity_memory_client_api.models import mental_model_trigger_input, update_mental_model_request
    ...
    def create_directive(
        ...
        Returns:
            DirectiveResponse
        """
        from singularity_memory_client_api.models import create_directive_request
    ...
    def update_directive(
        ...
        Returns:
            DirectiveResponse
        """
        from singularity_memory_client_api.models import update_directive_request
    ...
    def get_bank_config(self, bank_id: str) -> dict[str, Any]:
        ...
        Can be disabled on the server by setting ``SINGULARITY_ENABLE_BANK_CONFIG_API=false``.
