...
class GeminiEmbeddings(Embeddings):
    ...
    def _init_vertexai(self, genai) -> None:
        ...
        if not self.vertexai_project_id:
            raise ValueError(
                "SINGULARITY_EMBEDDINGS_VERTEXAI_PROJECT_ID (or SINGULARITY_LLM_VERTEXAI_PROJECT_ID) "
...
def create_embeddings_from_env() -> Embeddings:
    ...
    if provider == "tei":
        ...
        if not api_key:
            raise ValueError(
                "SINGULARITY_EMBEDDINGS_OPENROUTER_API_KEY, SINGULARITY_OPENROUTER_API_KEY, "
