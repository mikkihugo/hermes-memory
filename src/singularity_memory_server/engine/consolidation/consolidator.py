...
async def _find_related_observations(
    ...
    if is_tracing_enabled():
        recall_span = tracer.start_span("singularity_memory.consolidation_recall")
        recall_span.set_attribute("singularity_memory.bank_id", bank_id)
        recall_span.set_attribute("singularity_memory.query", query[:100])  # Truncate for brevity
        recall_span.set_attribute("singularity_memory.fact_type", "observation")
    else:
        recall_span = None
