...
Usage:
    SINGULARITY_LLM_PROVIDER=llamacpp
    SINGULARITY_LLAMACPP_MODEL_PATH=~/.singularity-memory/models/gemma-4-E2B-it-Q4_K_M.gguf
    SINGULARITY_LLAMACPP_GPU_LAYERS=-1  # -1 = all layers on GPU
    SINGULARITY_LLAMACPP_CONTEXT_SIZE=8192
"""
...
from singularity_memory_server.engine.llm_interface import LLMInterface
from singularity_memory_server.engine.response_models import LLMToolCallResult
...
MODELS_DIR = Path.home() / ".singularity-memory" / "models"
...
def _download_default_model() -> Path:
    ...
    try:
        ...
        raise ImportError(
            ...
            "Install with: pip install 'singularity-memory[local-llm]'"
        )
    ...
    if model_path:
        ...
        if not p.exists():
            raise FileNotFoundError(
                ...
                f"Set SINGULARITY_LLAMACPP_MODEL_PATH to a valid .gguf file, "
...
class LlamaCppLLM(LLMInterface):
    ...
    async def _ensure_initialized(self) -> None:
        ...
        if self._no_grammar:
            logger.info("Grammar enforcement disabled (SINGULARITY_LLAMACPP_NO_GRAMMAR=true)")
