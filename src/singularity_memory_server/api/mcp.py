"""Singularity Memory MCP Server implementation using FastMCP (HTTP transport)."""
...
from singularity_memory_server.extensions import MCPExtension, load_extension
from singularity_memory_server.extensions.tenant import AuthenticationError
from singularity_memory_server.mcp_tools import _ALL_TOOLS, MCPToolsConfig, register_mcp_tools
from singularity_memory_server.models import RequestContext

# Configure logging from SINGULARITY_LOG_LEVEL environment variable
_log_level_str = os.environ.get("SINGULARITY_LOG_LEVEL", "info").lower()
_log_level_map = {
...
DEFAULT_BANK_ID = os.environ.get("SINGULARITY_MCP_BANK_ID", "default")
...
MCP_AUTH_TOKEN = os.environ.get("SINGULARITY_MCP_AUTH_TOKEN")
...
def create_mcp_server(memory: MemoryEngine, multi_bank: bool = True) -> FastMCP:
    """
    Create and configure the Singularity Memory MCP server.
    ...
    """
    mcp = FastMCP("singularity-memory-mcp-server")
...
class MCPMiddleware:
    ...
    Authentication:
        1. If SINGULARITY_MCP_AUTH_TOKEN is set (legacy), validates against that token
    ...
    1. Multi-bank mode (for /mcp/ root endpoint):
       ...
       - Bank ID from: X-Bank-Id header or SINGULARITY_MCP_BANK_ID env var
    ...
    Bank ID resolution priority:
        ...
        3. SINGULARITY_MCP_BANK_ID env var → multi-bank mode (default: "default")
