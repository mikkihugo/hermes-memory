...
class WorkerPoller:
    ...
    async def _scan_active_schemas(self, schemas: list[str | None]) -> set[str | None]:
        ...
        In singularity-memory-cloud deployments this is installed by a Helm hook
    ...
    async def _log_progress_if_due(self):
        ...
        Emits four kinds of lines:
          ...
          * [DB_WAITS]      - any non-idle server session waiting on a lock
        """
        now = time.time()
    ...
    async def _log_db_waits(self) -> None:
        """Log any non-idle server session that's waiting on a lock or other resource.
