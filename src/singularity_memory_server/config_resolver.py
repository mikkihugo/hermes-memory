...
from singularity_memory_server.config import (
...
from singularity_memory_server.extensions.tenant import TenantExtension
...
class ConfigResolver:
    ...
    def __init__(self, pool: asyncpg.Pool, tenant_extension: TenantExtension | None = None):
        ...
        self._configurable_fields = SingularityConfig.get_configurable_fields()
        self._credential_fields = SingularityConfig.get_credential_fields()

    async def resolve_full_config(self, bank_id: str, context: RequestContext | None = None) -> SingularityConfig:
        """
        Resolve full SingularityConfig for a bank with hierarchical overrides applied.
        ...
        Returns:
            Complete SingularityConfig with hierarchical overrides applied
        """
        ...
        resolved_config = SingularityConfig(**config_dict)
    ...
    async def update_bank_config(
        ...
        Args:
            ...
            updates: Dict of config field names to new values.
                    Keys can be in env var format (SINGULARITY_LLM_PROVIDER)
        ...
        if invalid_fields:
            static_fields = SingularityConfig.get_static_fields()
...
def apply_strategy(config: SingularityConfig, strategy_name: str) -> SingularityConfig:
    """
    ...
    configurable = SingularityConfig.get_configurable_fields()
