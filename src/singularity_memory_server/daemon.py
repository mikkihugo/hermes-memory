"""
Daemon mode support for Singularity Memory.
...
DEFAULT_IDLE_TIMEOUT = 0  # 0 = no auto-exit (singularity-memory-embed passes its own timeout)

# Allow override via environment variable for profile-specific logs
DAEMON_LOG_PATH = Path(os.getenv("SINGULARITY_DAEMON_LOG", str(Path.home() / ".singularity-memory" / "daemon.log")))
