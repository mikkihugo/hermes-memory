"""
OpenTelemetry distributed tracing instrumentation for Singularity Memory.
...
# Provider name mapping (Internal -> GenAI semantic convention)
...
def create_operation_span(operation: str, bank_id: str | None = None):
    """
    Create a parent span for a Singularity Memory operation (retain, reflect, consolidation, etc.).

    This creates the span hierarchy:
    - singularity_memory.{operation} (parent)
    ...
    span_name = f"singularity_memory.{operation}"
    span = _tracer.start_as_current_span(span_name)

    # Add operation-specific attributes
    if span and hasattr(span, "set_attribute"):
        span.set_attribute("singularity_memory.operation", operation)
        if bank_id:
            span.set_attribute("singularity_memory.bank_id", bank_id)
...
class LLMSpanRecorder:
    ...
    def record_llm_call(
        ...
        try:
            ...
            # Create span name: "singularity_memory.{scope}" for consistency with parent spans
            # Model info is available in span attributes (gen_ai.request.model)
            if scope:
                span_name = f"singularity_memory.{scope}"
            else:
            ...
            with self.tracer.start_as_current_span(
                ...
                # Add custom attributes for Singularity Memory context
                span.set_attribute("singularity_memory.scope", scope)
                span.set_attribute("singularity_memory.provider.internal", provider)
