"""
Command-line interface for Singularity Memory.

Run the server with:
    singularity-memory serve

Run as background daemon:
    singularity-memory --daemon
...
from .singularity_config import DEFAULT_WORKERS, ENV_WORKERS, SingularityConfig, _get_raw_config
from .daemon import (
    DEFAULT_DAEMON_PORT,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        description="Singularity Memory Server",
    )

    # Server options
    parser.add_argument(
        "--host", default=config.host, help=f"Host to bind to (default: {config.host}, env: SINGULARITY_HOST)"
    )
    parser.add_argument(
        "--port",
        type=int,
        default=config.port,
        help=f"Port to bind to (default: {config.port}, env: SINGULARITY_PORT)",
    )
    parser.add_argument(
        "--log-level",
    ...
    uvicorn_config = {
        "app": "singularity_memory_server.server:app" if use_import_string else app,
        "host": args.host,
        "port": args.port,
