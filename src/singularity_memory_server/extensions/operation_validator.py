...
if TYPE_CHECKING:
    from singularity_memory_server.engine.memory_engine import Budget
    from singularity_memory_server.engine.response_models import RecallResult as RecallResultModel
    from singularity_memory_server.engine.response_models import ReflectResult
...
class OperationValidatorExtension(Extension, ABC):
    ...
    Configuration is passed from prefixed environment variables:
        SINGULARITY_OPERATION_VALIDATOR_MAX_REQUESTS=100
    ...
    Outcomes for `validate_*` hooks:
        ...
        - defer: raise `DeferOperation(exec_date, reason)` from
          `singularity_memory_server.worker.exceptions` to requeue the task for a
