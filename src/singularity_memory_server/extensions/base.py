"""Base Extension class for all Singularity Memory extensions."""

from abc import ABC
from typing import TYPE_CHECKING

if TYPE_CHECKING:
    from singularity_memory_server.extensions.context import ExtensionContext


class Extension(ABC):
    """
    Base class for all Singularity Memory extensions.

    Extensions are loaded via environment variables and receive configuration
    from prefixed environment variables.

    Example:
        SINGULARITY_MY_EXTENSION=mypackage.ext:MyExtension
        SINGULARITY_MY_SOME_CONFIG=value
