"""
Singularity Memory Admin CLI - backup and restore operations.
"""
...
from ..config import DEFAULT_DATABASE_SCHEMA, SingularityConfig
...
app = typer.Typer(name="singularity-memory-admin", help="Singularity Memory administrative commands")
...
def backup(
    ...
    """Backup the Singularity Memory database to a zip file."""
    config = SingularityConfig.from_env()

    if not config.database_url:
        typer.echo("Error: Database URL not configured.", err=True)
        typer.echo("Set SINGULARITY_DATABASE_URL environment variable.", err=True)
        raise typer.Exit(1)
